...
if enableLog == True:
    try:   
        ...
        if os.path.isdir(logFolder) == False:
            os.mkdir(logFolder)
        logFileName = os.path.abspath(os.path.join(curpath, logFolder , logFilePrefix + currentTimestamp + logFileExtension))
        logging.basicConfig(filename=logFileName, filemode='a', format=loggerMessageFormat, level=logging.DEBUG, datefmt=loggerDateFormat)
